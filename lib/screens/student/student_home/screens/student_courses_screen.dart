...
class StudentCoursesScreen extends StatefulWidget {
  ...
  StudentCoursesScreen({required this.studentStudyInfoId});
...
class _StudentCoursesScreenState extends State<StudentCoursesScreen> {
  late StudentAuthCubit _authCubit;
  late Connectivity _connectivity;

  @override
  void initState() {
    super.initState();
    _authCubit = StudentAuthCubit(ApiService());
  ...
  Future<void> _checkInternetConnection() async {
    ...
    if (result == ConnectivityResult.none) {
      ...
      _authCubit.getStudentCourses(widget.studentStudyInfoId);
    }
  }
        ...
        child: Scaffold(
          ...
          body: BlocConsumer<StudentAuthCubit, StudentAuthState>(
            listener: (context, state) {
              if (state is CourseSaved || state is AuthError) {
            ...
            builder: (context, state) {
              if (state is AuthLoading) {
                ...
                if (state.courses.isEmpty) {
                  return Center(
                      child: Text('لا توجد مقررات يمكن اضافة المقررات من خلال الضغط على الزر +'));
                } else {
                        ...
                        child: ListTile(
                          ...
                          subtitle: Padding(
                            ...
                            child: CircularPercentIndicator(
                              ...
                              progressColor: getColorForPercentage(attendancePercentage),
                            ),
                          ),
                          trailing: IconButton(
                            iconSize: 30.0,
          ...
          floatingActionButton: FloatingActionButton(
            onPressed: () {
              showDialog(
                context: context,
                builder: (context) {
                  return AddCourseDialog(studentStudyInfoId: widget.studentStudyInfoId);
                },
              );
            },
